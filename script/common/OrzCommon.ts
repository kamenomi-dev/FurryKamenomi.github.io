var
  sha256= (data: string, isBigCase: boolean = true): string => {
    let
      /* Function */
    ...
    let core_sha256 = (m, l) => {
      let
        K = new Array(0x428A2F98, 0x71374491, 0xB5C0FBCF, 0xE9B5DBA5,
    ...
    function str2binb(data: string): number[] {
      let
        bin: number[] = Array(),
        mask: number = (1 << 8) - 1;
    ...
    function binb2hex(binarray: number[]): string {
      let
        hexTab: string = isBigCase ? "0123456789ABCDEF" : "0123456789abcdef",
        strResult: string = "";
  ...
  formatTime = (format: string = 'yy-MM-dd HH:mm:ss.SS'): string => {
    let
      dt = new Date(),
      y = dt.getFullYear(),
      d = dt.getDate(),
    ...
    function zeroFill(data: number, maxLength: number): string {
      let string: string = '';
      for (let a = 0; a < maxLength - data.toString().length; a++) {
        string += '0'
      };
  ...
  getCaller = (step: number = 3): { callerFn: string, fileName: string, position: { col: number, row: number } } | void => {
    let
      errStack: string = '',
      callerLog: string[] = [],
      callerLine: string[] = [];
